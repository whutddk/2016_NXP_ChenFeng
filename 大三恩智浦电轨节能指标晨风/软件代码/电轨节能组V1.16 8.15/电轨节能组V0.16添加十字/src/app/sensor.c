...
void ramp()
{
  ...
  //push(12,LDC.data_sum);
   ...
   temp_threshold = ((LDC.max[0] + LDC.max[1])/ 2 ) * 2.5;
  
  if(LDC.data_buff[0] > temp_threshold && LDC.data_buff[i]<5000)
    for(i=0;i<6;i++)
    {
      if(LDC.data_buff[i] > (temp_threshold - 100)  && LDC.data[3] > (LDC.max[0] * 1.5))
        j++;
    }
  else if(LDC.data_buff[0] < LDC.max[0] + 80)
    for(i=0;i<6;i++)
    {
      if(LDC.data_buff[i] < (LDC.max[0] + 180))
        k++;
    }
