...
{
  LDC_SEND(REF_COUNT_CH0,0x36b0);//转化时间，Conversion Time (tC0) =(CH0_RCOUNT?16)/fREF0
  
  
  LDC_SEND(REF_COUNT_CH1,0x36b0);
  
  
  LDC_SEND(REF_COUNT_CH2,0x36b0);
  
  
  LDC_SEND(REF_COUNT_CH3,0x0010);
  
  
  LDC_SEND(OFFSET_CH0,0x0180);//频率偏置f offset 为0
  

  LDC_SEND(OFFSET_CH1,0x0180);
  

  LDC_SEND(OFFSET_CH2,0x0180);
  ...
  LDC_SEND(SETTLECOUNT_CH0,0x0800);//0x0080数字大保证稳定时间长
  ...
  LDC_SEND(DRIVE_CURRENT_CH0,0x39c0);//线圈105*652940
  

  
  LDC_SEND(DRIVE_CURRENT_CH1,0x39c0);
  

  
  LDC_SEND(DRIVE_CURRENT_CH2,0x3180);
  

  
  LDC_SEND(DRIVE_CURRENT_CH3,0x0800);
