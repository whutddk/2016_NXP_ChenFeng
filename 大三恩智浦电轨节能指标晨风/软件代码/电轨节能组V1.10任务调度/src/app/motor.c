...
void motor_out()
{
  ...
  tpm_pwm_duty(TPM0, TPM_CH0, (uint32)ctl.left.core.out);
  tpm_pwm_duty(TPM0, TPM_CH3, (uint32)ctl.rigt.core.out);
}
...
void ASR()
{
  ...
  ctl.left.shell.error[0] = ctl.left.shell.aim - ctl.left.shell.cur ;
  ctl.rigt.shell.error[0] = ctl.rigt.shell.aim - ctl.rigt.shell.cur ;
  
  ctl.left.shell.result = ctl.left.shell.Kp * (ctl.left.shell.error[0] - ctl.left.shell.error[1]);
  ctl.left.shell.result += ctl.left.shell.Ki * ctl.left.shell.error[0];
  ctl.rigt.shell.result = ctl.rigt.shell.Kp * (ctl.rigt.shell.error[0] - ctl.rigt.shell.error[1]);
  ...
  push(0, ctl.left.shell.cur);
  push(1, ctl.rigt.shell.cur);
}
